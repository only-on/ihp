    ...
    <div class="head">
      <Row>
        ...
        <div class="head-title">
          ...
          <span style="margin-right: 20px;font-size: 16px;color: #666666">编辑合作社</span>
        </div>
        </Col>
      </Row>
    </div>
...
<script>
import { cooperativeDetail, companyList, editCooperation } from '../../../api/api'
  ...
  methods: {
    ...
    submit () {
      if (this.submitVal()) {
        ...
        editCooperation(para).then((res) => {
          if (res.data.code === 1000) {
            this.$router.push({name: 'cooperative_manage', params: {param: this.$route.params.param}})
            this.$Message.success('合作社修改成功！')
          } else {
