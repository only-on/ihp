    ...
    <div class="head">
      <Row>
        ...
        <div class="head-title">
          ...
          <span style="margin-right: 20px;font-size: 16px;color: #666666">创建烤房群</span>
        </div>
...
<script>
import { addTobocco } from '../../../api/api'
export default {
  data () {
    return {
      addressDetail: '',
    ...
    submit () {
      if (this.submitManVal()) {
        this.$refs['updateModel'].validate((valid) => {
          if (valid) {
            addTobocco(this.updateModel).then((res) => {
              if (res.data.code === 1000) {
                // console.log(JSON.stringify(this.updateModel))
                this.$router.push({name: 'tobaccoGroup_manage'})
                this.$Message.success('烤烟群创建成功！')
              } else {
