    ...
    <div class="head">
      <Row>
        ...
        <div class="head-title">
          ...
          <span style="margin-right: 20px;font-size: 16px;color: #666666">新增烤房</span>
        </div>
        </Col>
      </Row>
    </div>
...
<script>
import { companyList, cooperativeList, addTobacco } from '../../../api/api'
  ...
  methods: {
    ...
    submit () {
      if (this.submitVal()) {
        ...
        addTobacco(para).then((res) => {
          if (res.data.code === 1000) {
            this.$router.push({name: 'tobacco_manage'})
            this.$Message.success('烤房创建成功！')
          } else {
