    ...
    <div class="head">
      <Row>
        ...
        <div class="head-title">
          ...
          <span style="margin-right: 20px;font-size: 16px;color: #666666">新增告警</span>
        </div>
        </Col>
      </Row>
    </div>
...
export default {
  ...
  methods: {
    ...
    submit () {
      // console.log(typeof this.updateModel.number)
      if (this.submitVal()) {
        this.$refs['updateModel'].validate((valid) => {
          if (valid) {
            alarmAdd(this.updateModel).then((res) => {
