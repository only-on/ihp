    ...
    <div class="head">
      <Row>
        ...
        <div class="head-title">
          ...
          <span style="margin-right: 20px;font-size: 16px;color: #666666">编辑用户</span>
        </div>
        <Button  @click="back" style="background-color: #848484;color:#fff !important;width: 100px;float: right;margin-top: 8px">返回</Button>
        </Col>
      </Row>
    </div>
    <div class="content">
      <div class="layout-content-form">
        <Form :model="updateModel" :label-width="150" ref="updateModel" :rules="ruleValidate">
          ...
          <div>
            ...
            <Row type="flex" justify="center">
              <Col span="23" push="1">
              <FormItem label="更改密码"  prop="password">
                <Input  v-model="updateModel.password" placeholder="更改密码 " style="width: 200px; float: left" @on-change="passwordVal(updateModel.password)"></Input>
...
<script>
import { uploadImg, updateUser, cooperativeList, companyList, getUserDetail } from '../../../api/api'
  ...
  methods: {
    ...
    submit () {
      if (this.submitVal()) {
        ...
        updateUser(this.updateModel).then((res) => {
          if (res.data.code === 1000) {
    ...
    getUserDetail(param).then((res) => {
      ...
      if (res.data.code === 1000) {
        ...
        if (res.data.content.regionIds.length > 0) {
          ...
          this.cName = res.data.content.regionVO.cityName
          this.dName = res.data.content.regionVO.districtsName
          ...
          for (let i = 0; i < arr.length; i++) {
            let oo = {
              ...
              cooperationId: arr[i].cooperationId,
              index: i,
              status: 1,
              cooperativeList: []
            }
            ...
            cooperativeList(par).then((res) => {
              if (res.data.code === 1000) {
                oo.cooperativeList = res.data.content
              } else {
                oo.cooperativeList = []
              }
            })
